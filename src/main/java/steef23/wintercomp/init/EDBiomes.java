...
import steef23.wintercomp.world.biome.WinterHorrorBiome;
import steef23.wintercomp.world.biome.WinterHorrorSurfaceBuilder;

public class EDBiomes 
{
	public static final DeferredRegister<Biome> BIOMES = new DeferredRegister<>(ForgeRegistries.BIOMES, WinterComp.MOD_ID);
			...
			() -> new WinterHorrorBiome(new Biome.Builder()
					.surfaceBuilder(new ConfiguredSurfaceBuilder<SurfaceBuilderConfig>(
							...
							new SurfaceBuilderConfig(Blocks.COARSE_DIRT.getDefaultState(),
									...
									Blocks.DIRT.getDefaultState())))
					.precipitation(RainType.SNOW).category(Category.NONE).depth(0.1f).scale(0.4f).temperature(1.0f).downfall(1.0f).waterColor(0x000000).waterFogColor(0x461917).parent(null)));
